...
export default class Web3Connection {
  ...
  suggestNetworkSwitch = async (network) => {
    ...
    try {
      ...
      console.log(
        "Could not switch network. You might already be connected to this network or your browser only support manual network switching."
      );
  ...
  fetchTokenImage = async (address, tokenId) => {
    ...
    let nftOwnerAddress;
    try {
      nftOwnerAddress = await this.nftContract.ownerOf(
    ...
    let tokenUri;
    try {
      tokenUri = await this.nftContract.tokenURI(
        ethers.BigNumber.from(tokenId)
      );
    } catch (e) {
      console.log(e);
